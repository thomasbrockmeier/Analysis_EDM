...
rt.matrix <- function(desc.r, desc.t, thresh1, thresh2, thresh3, thresh4, pairs, len, category) {
        std <- 2
        ...
        if (length(pair.number) >= len+1){
                while (length(pair.number) >= len+1){
        ...
        as.data.frame(cbind(pair.number, Segment1, Segment2, category, Rhythm.n, Timbre.n, Rhythm.mean, Timbre.mean, Rhythm.sd, Timbre.sd))
#         fin <-as.data.frame(cbind(pair.number, Segment1, Segment2, category, Rhythm.n, Timbre.n, Rhythm.mean, Timbre.mean, Rhythm.sd, Timbre.sd))
#         fin[!duplicated(fin[, "Segment1"]) | duplicated(fin[, "Segment2"]), ]
}

hrht <- rt.matrix(desc.r, desc.t, thresh1=3, thresh2=4, thresh3=3, thresh4=4, pairs, 10, 'HRHT')
hrlt <- rt.matrix(desc.r, desc.t, thresh1=3, thresh2=4, thresh3=1, thresh4=2, pairs, 10, 'HRLT')
lrht <- rt.matrix(desc.r, desc.t, thresh1=1, thresh2=2, thresh3=3, thresh4=4, pairs, 100, 'LRHT')
lrlt <- rt.matrix(desc.r, desc.t, thresh1=1, thresh2=2, thresh3=1, thresh4=2, pairs, 10, 'LRLT')
hrmt <- rt.matrix(desc.r, desc.t, thresh1=3, thresh2=4, thresh3=2.2, thresh4=2.8, pairs, 1, 'HRMT')
mrht <- rt.matrix(desc.r, desc.t, thresh1=2.2, thresh2=2.8, thresh3=3, thresh4=4, pairs, 1, 'MRHT')
mrmt <- rt.matrix(desc.r, desc.t, thresh1=2.2, thresh2=2.8, thresh3=2.2, thresh4=2.8, pairs, 10, 'MRMT')
lrmt <- rt.matrix(desc.r, desc.t, thresh1=1, thresh2=2, thresh3=2.2, thresh4=2.8, pairs, 1, 'LRMT')
mrlt <- rt.matrix(desc.r, desc.t, thresh1=2.2, thresh2=2.8, thresh3=1, thresh4=2, pairs, 1, 'MRLT')

final <- rbind(hrht, hrlt, lrht, lrlt, hrmt, mrht, mrmt, lrmt, mrlt)
# write.csv(final, 'final_listv5.csv')
